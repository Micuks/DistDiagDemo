...
const formatValue = (value, category, metric) => {
  ...
  if (
    ...
    if (value < 1000 * 1000) return (value / 1000).toFixed(2).toLocaleString(); // ms
    return (value / (1000 * 1000)).toFixed(2).toLocaleString(); // s
  }
  ...
  if (
    typeof metric === "string"
      ...
      : metric.name.includes("write size") ||
        metric.name.includes("log total size")
  ) {
    if (value < 1024) return `${value.toFixed(0)}`; // Bytes
    if (value < 1024 * 1024) return `${(value / 1024).toFixed(2)}`; // KB
    if (value < 1024 * 1024 * 1024)
...
const MetricsPanel = () => {
  ...
  const fetchChartData = async (nodeIp, category, metric) => {
    ...
    try {
      ...
      console.error(
        `Error fetching ${category} chart data for ${nodeIp}:`,
        error
      );
  ...
  const fetchAllNodeMetrics = async (nodeIp) => {
    ...
    try {
      ...
      console.error(
        `Error fetching all detailed metrics for ${nodeIp}:`,
        error
      );
    } finally {
  ...
  const getMetricSuffix = (category, metric) => {
    ...
    if (
      metricName.includes("write size") ||
      metricName.includes("log total size")
    ) {
      if (metricValue < 1024) return "B";
      if (metricValue < 1024 * 1024) return "KB";
      if (metricValue < 1024 * 1024 * 1024) return "MB";
  ...
  const renderTimeSeriesChart = (nodeIp, category, metric) => {
    ...
    if (chartData.length === 0) {
      console.warn(
        `Empty data array for chart ${nodeIp}/${category}/${metric}`
      );
      return (
        <div style={{ textAlign: "center", padding: "20px", height: "200px" }}>
          No data available
      ...
      >
        ...
        <ResponsiveContainer>
          ...
          >
            ...
            <Tooltip
              formatter={(value) => [
                `${formatValue(value, category, metric)} ${suffix}`,
                metric,
              ]}
  ...
  const renderMetricItem = (nodeIp, category, metricName) => {
    ...
    console.debug(
      `Rendering metric item ${nodeIp}/${category}/${metricName}:`,
      metric
    );
  ...
  const renderNodeMetrics = (nodeIp, nodeData = {}) => {
    ...
    const cpu =
      nodeData.cpu && typeof nodeData.cpu === "object" ? nodeData.cpu : {};
    const memory =
      nodeData.memory && typeof nodeData.memory === "object"
        ? nodeData.memory
        : {};
    const io =
      nodeData.io && typeof nodeData.io === "object" ? nodeData.io : {};
    const network =
  ...
  const getFluctuationStyle = (metric) => ({
    ...
    boxShadow: metric.has_fluctuation
      ? "0 2px 8px rgba(255, 214, 102, 0.1)"
      : "none",
    padding: "12px",
    marginBottom: "8px",
      ...
      <Card title="System Metrics" style={{ marginBottom: 16 }}>
        {loading ? (
          ...
          <>
            ...
            {metricPoint?.metrics &&
              Object.entries(metricPoint.metrics).map(([nodeIp, nodeData]) =>
